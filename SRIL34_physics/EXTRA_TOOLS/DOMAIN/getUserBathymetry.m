...
zfields={'Z','h','z','zbathy','depth','Depth','DEPTH','Elevation','Band1','depths','topo_depth5','elevation'};
unread=true;
...
%%jholt
if length(zbathy)== length(xbathy) .* length(ybathy)
lon=xbathy;
lat=ybathy;
xbathy=lon*ones(1,length(lat));
xbathy=xbathy(:);
ybathy=(lat*ones(1,length(lon)))';
ybathy=ybathy(:);
end
